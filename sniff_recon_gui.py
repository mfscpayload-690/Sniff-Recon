...
def inject_modern_css():
    ...
    st.markdown(
        ...
        /* Modal (popup) styles with typewriter */
        .sr-modal-overlay {
        ...
        .sr-modal {
            ...
            background: linear-gradient(145deg, rgba(18, 18, 30, 0.98), rgba(10, 10, 20, 0.98));
            border: 2px solid rgba(0,255,255,0.4);
            box-shadow: 0 0 40px rgba(0,255,255,0.25), inset 0 0 20px rgba(0,255,255,0.08);
...
def main():
    ...
    # Sidebar: About and Help Desk buttons only (simple, no expanded content)
    with st.sidebar:
    ...
    # Main title with original goggles icon restored
    st.markdown(
    ...
    # Modal renderer with typewriter for content
    ...
    if modal:
        ...
        if modal_type == "about":
            ...
            body = (
                ...
                "It features optional multi-provider AI analysis, memory-aware parsing, and clear visualizations to help you investigate network traffic quickly and safely."
            )
        ...
        # Render overlay and modal with typewriter
        ...
        # Typewriter animation for modal body
        ...
        if not st.session_state.get(f"{modal_id}_typed"):
            st.markdown(
                ...
                <script>
                  (function() {{
                      const txt = {json.dumps(body)};
                      const el = window.parent.document.getElementById('{modal_id}');
