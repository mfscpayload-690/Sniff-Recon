...
def inject_modern_css():
    st.markdown(
        ...
        /* ==================== MINIMAL PURE BLACK BACKGROUND ==================== */
        .stApp, .main, body, html { background:#000 !important; }
        .main { font-family:'Inter',sans-serif; position:relative; padding:2rem; color:#e0e0e0; }
        /* Remove ALL pseudo-element decorative layers from prior iterations */
        .main::before, .main::after, body::before, body::after, html::before, html::after, .stApp::before, .stApp::after { content:none !important; display:none !important; }
        /* Ensure content stacks above starfield canvas */
...
def main():
    ...
    # Inject starfield canvas + animation script (pure black bg, cyan glow)
    st.markdown(
        """
        ...
        (function(){
            ...
            let stars = [];
            const cyanPalette = ['#00ffff','#00e6ff','#00d4ff','#4ad3b0'];
            const prefersReduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
            function resize(){
                canvas.width = window.innerWidth; canvas.height = window.innerHeight;
                const area = canvas.width * canvas.height;
                const target = Math.min(140, Math.max(60, Math.floor(area/15000))); // responsive clamp
                stars = Array.from({length: target}, ()=>({
                    ...
                    c: cyanPalette[Math.floor(Math.random()*cyanPalette.length)]
                }));
            }
            function step(){
            ...
            resize();
            step();
            // In reduced motion mode we only draw once (static stars)
        })();
        </script>
        <style>
